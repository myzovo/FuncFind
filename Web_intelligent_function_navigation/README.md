...
- `--baseline` / `BASELINE_PATH`：上一份结果文件，用于 checksum 对比
  - `--baseline` / `BASELINE_PATH`: baseline dataset for checksum diff
...
- `contextText`：元素附近的上下文文本摘要
  - `contextText`: nearby context text snippet
- `nearestHeading`：最近的标题或区块标题
  - `nearestHeading`: nearest heading text
- `tag`：元素标签名（button/a/input 等）
  - `tag`: element tag name
- `role`：ARIA role（若存在）
  - `role`: aria role if present
- `xpath`：元素 XPath（用于定位的备选）
  - `xpath`: xpath fallback for locating element
...
## 推送到 RAG（可选）/ Push to RAG (Optional)

控制台新增 **RAG 构建** 面板，可将爬取结果发送到 DocVec_RAG 的
`/api/knowledge-base/build-from-crawl` 接口。

The console includes a **RAG Build** panel to send crawl results to the
DocVec_RAG `/api/knowledge-base/build-from-crawl` endpoint.

常用字段 / Common fields:

- `RAG Base URL`: RAG 后端地址（例如 `http://localhost:8080`）
- `KB Name`: 知识库名称
- `Dataset`: `latest` 或 database 目录中的 JSON 文件名
- `Session ID`（可选）: 对应 `X-Client-Session-Id`
- `Embedding Model`（可选）: 例如 `all-MiniLM-L6-v2`
