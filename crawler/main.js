...
const baselinePathArg = getArg("--baseline") || process.env.BASELINE_PATH;
const baselinePath = baselinePathArg ? resolveDatabasePath(baselinePathArg) : "";
...
const baselineData = loadBaselineData(baselinePath || databasePath);
const baselineByUrl = buildBaselineIndex(baselineData);
if (baselineByUrl.size) {
  log.info(`Loaded baseline dataset for incremental crawl: ${baselinePath || databasePath}`);
}
...
const crawler = new PlaywrightCrawler({
  ...
  requestHandler: async ({ request, page, enqueueLinks, log: pageLog }) => {
    ...
    const checksum = hashContent(pageHtml);

    const baseline = baselineByUrl.get(pageUrl);
    let elements = [];
    let title = await page.title().catch(() => "");

    if (baseline && baseline.checksum === checksum) {
      elements = Array.isArray(baseline.elements) ? baseline.elements : [];
      if (!title && baseline.title) {
        title = baseline.title;
      }
      pageLog.info(`Page unchanged, reusing baseline: ${pageUrl}`);
    } else {
      elements = await extractInteractiveElements(page, allowedDomains, sensitiveWords);
    }

    if (!pagesByUrl.has(pageUrl)) {
      pagesByUrl.set(pageUrl, { url: pageUrl, title, elements, checksum });
    }
...
function buildBaselineIndex(data) {
  const map = new Map();
...
async function extractInteractiveElements(page, allowedDomains, sensitiveWords) {
  return page.evaluate((payload) => {
    ...
    function getContextText(el) {
      const container =
        (el.closest && el.closest("section,article,main,aside,nav,div,li,td,th")) || el.parentElement;
      if (!container) return "";
      const text = (container.innerText || container.textContent || "").replace(/\s+/g, " ").trim();
      return text.slice(0, MAX_CONTEXT_LEN);
    }
    ...
    function getNearestHeading(el) {
      ...
      while (current && hops < 6) {
        if (current.closest) {
          const closest = current.closest("h1,h2,h3,h4,h5,h6,[role='heading']");
          if (closest) {
            return getHeadingText(closest);
          }
        }

        const candidate =
          current.querySelector &&
          current.querySelector("h1,h2,h3,h4,h5,h6,[role='heading'],.section-title,.section-header");
        if (candidate) {
          return getHeadingText(candidate);
        }
    ...
    function getXPath(el) {
      ...
      while (current && current.nodeType === 1 && current !== root && hops < 10) {
        const tag = current.tagName.toLowerCase();
        let index = 1;
