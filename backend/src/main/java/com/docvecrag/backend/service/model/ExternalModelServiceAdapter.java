...
public class ExternalModelServiceAdapter implements ModelAdapter {
    ...
    private String buildFallbackAnswer(String question, List<RetrievedChunk> contexts) {
        ...
        builder.append("原因：LLM API 调用失败或未配置，以下仅为知识库检索结果：\n\n");
        ...
        builder.append("- External 模式：确保 `baseUrl` 和 `apiKey` 已正确配置\n");
        builder.append("- Workers AI 模式：确保 Cloudflare `accountId` 和 `apiToken` 已配置\n");
